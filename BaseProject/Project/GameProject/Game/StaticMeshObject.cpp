...
StaticMeshObject::StaticMeshObject(UpdatePriority _updatePriority, const char* _name, const char* _modelName, RenderPriority _renderPriority):
Base(_updatePriority, _name), mp_ParentModel(nullptr)
{
	m_Model = COPY_RESOURCE(_modelName, CModelObj);

	m_Render.RegistRender(this, (RenderFunc)&StaticMeshObject::Render, _renderPriority, _modelName);
}
...
void StaticMeshObject::SetTransform(const Transform& transform)
{
	this->m_Transform = transform;
}

void StaticMeshObject::SetParent(CModel* p, int idx) {
...
CModelObj& StaticMeshObject::GetModel()
{
	return m_Model;
}
