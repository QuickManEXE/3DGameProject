...
CMeshFBX::CMeshFBX(int no, FbxMesh* pMesh, FbxScene* pScene, CModelFBX* fbx) : CFrameFBX(no, pMesh->GetNode()) {
	unsigned int pointCnt = pMesh->GetControlPointsCount();
	unsigned int uvCnt = pMesh->GetTextureUVCount();
	unsigned int polyCnt = pMesh->GetPolygonCount();
	unsigned char* polygonSize = new unsigned char[polyCnt];
	...
	if (uvCnt && uv->GetMappingMode() == FbxLayerElement::eByControlPoint) {
		FbxLayerElementUV* pUV = pMesh->GetLayer(0)->GetUVs();
		for (unsigned int k = 0; k<uvCnt; k++) {
	...
	for (int i = 0; i< m_material_cnt; i++) {
		...
		for (unsigned int k = 0; k< polyCnt; k++) {
	...
	if (m_numBone > 0) {
		...
		for (unsigned int i = 0; i < m_vertexCnt; i++) {
			float w = 0;
			for (int j = 0; j < 4; j++) {
...
void CMeshFBX::Render(CShader *s, std::vector<CMaterial*>& materialList,CMatrix* boneMatrix,float blend) {
	int	offset = 0;
	...
	if (!m_vertexBuffer) {
		...
		for (int i = 0; i < m_material_cnt; i++) {
			...
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_numFace[i] * 3 * sizeof(int), mp_Indexs[i], GL_STATIC_DRAW);
			delete[] mp_Indexs[i];
		}
	...
	for (unsigned int i = 0; i < m_material_cnt; i++) {
		CMaterial* m = materialList[m_material[i]];
		m->Map(s);
...
//			m = m * pFrameFBX->localMatrix;
		...
		//	glGetFloatv(GL_MODELVIEW_MATRIX, mModelView.f);
			...
			glUniformMatrix4fv(glGetUniformLocation(m_shader->GetProgram(), "WorldMatrix"), 1, false, m.f);

			CMeshFBX* pMeshFBX = (CMeshFBX*)pNode->GetUserDataPtr();
			pMeshFBX->Render(m_shader, m_materialList, m_boneMatrix,m_blend);
	...
	if (anim_size > 0) {
		for (int i = 0; i < anim_size; i++) {
			m_animation_list.push_back(new AnimationInfo(m_lSdkManager, m_lScene,
				m_lScene->GetSrcObject<FbxAnimStack>(i)));
		}
...
void CModelFBX::Render(CMatrix &m) {
	...
	m_shader->enable();
	SendShaderParam(m_matrix, CCamera::GetCurrent()->GetViewMatrix(), CCamera::GetCurrent()->GetProjectionMatrix());
...
void CModelFBX::ChangeAnimation(int jam, bool loop, bool check, float time, int blendFrame) {
	if (!m_pAnimData && jam >=m_anim_size) return;
	if (check && m_animJam == jam) return;
	m_animJam = jam;
	m_animLoop = loop;
	m_blendvec = (blendFrame>0) ? 1.0f / blendFrame : 1.0f;
	m_blend = 0;
...
bool CModelFBX::AddAnimation(const char* filePath) {
	...
	for (int i = 0; i < anim_size; i++, m_anim_size++) {
		m_animation_list.push_back(new AnimationInfo( a->m_lSdkManager,a->m_lScene,
			a->m_lScene->GetSrcObject<FbxAnimStack>(i)));
	}
