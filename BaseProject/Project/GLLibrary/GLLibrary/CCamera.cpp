...
void CCamera::SetCurrent(CCamera *p) {
	m_pCurrent = p;
	glViewport(p->m_viewportRect.m_x, p->m_viewportRect.m_y, p->m_viewportRect.m_width, p->m_viewportRect.m_height);
}
//コンストラクタ
CCamera::CCamera(){
}
...
void CCamera::Viewport(int x, int y, int w, int h)
{
