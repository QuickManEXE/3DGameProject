...
//	CA3MMesh(const CA3MMesh& mesh);
	...
	描画
	*/
	void Draw(std::vector<CMaterial*>& materialList, const CShader* shader,CMatrix* send_matrix, CMatrix* bone_matrix_base,int bone_size);
	void CreateCollisionIndex(const CA3MColl* c);
...
public:
	...
	~CA3MAnimation();
	bool ChangeAnimation(int jam, bool loop, bool check, float time, float blend_frame);
...
public:
	...
	**/
	...
	void ChangeAnimation(int layer, int jam, bool loop = true, bool check = true, float time = 0, float blendtime = 0.1);
