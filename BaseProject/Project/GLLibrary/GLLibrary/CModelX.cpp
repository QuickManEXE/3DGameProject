...
CMaterialX::CMaterialX(CXMaterial &m, char *path) {
	...
	if (strlen(m.filename)) {
		strcpy_s(m_texture_name, NAME_STR_SIZE, m.filename);
		m_pTex = new CTexture();
		char str[256];
		strcpy_s(str, 256, path);
		strcat_s(str, 256, m.filename);

		if (!m_pTex->Load(str) && !m_pTex->Load(m.filename)) {
			delete m_pTex;
			m_pTex = NULL;
		}
	}
...
CMaterialX::CMaterialX(const CMaterialX& mesh) {
	memcpy(this, &mesh, sizeof(CMaterialX));
}
...
void CXMesh::draw(std::vector<CMaterialX*> &materialList, CShader *shader, CFrameBone *pFrameRoot, CMatrix *boneMatrix, int boneNum) {
	if (shader) {
		if (boneList.size()>0) {
...
void CModelX::operator = (const CModelX &m) {
	...
	for (auto it = m.pMaterialList->begin(); it != m.pMaterialList->end(); it++) {
		pMaterialList->push_back(new CMaterialX(**it));
	}
