...
const char* effect_vart = "#version 430\n"\
"layout(location = 0) in vec3 Vertex;\n"\
"out vec2 texCoord;\n"\
"void main(void)\n"\
"{\n"\
...
const char* noeffect_frag = "#version 430\n"\
"uniform sampler2D tDiffuse;\n"\
"in vec2 texCoord;\n"\
"out vec4 out_color;\n"\
"void main() {\n"\
...
const char* depth_vert = "#version 430\n"\
"layout(location = 0) in vec3 Vertex;\n"\
"out vec2 texCoord;\n"\
"void main(void)\n"\
"{\n"\
...
"	gl_Position = vec4(Vertex.xy * 2.0 - 1.0, 0.0, 1.0);\n"\
"}\n";
const char* depth_flag = "#version 430\n"\
...
CShader::CShader(std::string type) {
	...
	if (type == "Effect") {
		loadraw(effect_vart, effect_frag);	
	}else
	if (type == "NoEffect") {
		loadraw(effect_vart, noeffect_frag);
	}else
	if (type == "Depth") {
		loadraw(depth_vert, depth_flag);
	}else
	if (type == "Image") {
