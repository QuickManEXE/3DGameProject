...
void CA3MPoly::CreateCollisionIndex(const CA3MColl*c) {
	...
	for (int i = 0; i < m_vertex_cnt; i += 3, v += 3) {
...
bool CA3MPoly::CollisionRay(CVector3D * c, CVector3D * n, const CVector3D & s, const CVector3D & e, const CVector3D & dir, float* pLength, const CVector3D &start, const CVector3D& end) {
	...
	memset(m_colmask, 0, sizeof(unsigned long) * ((m_vertex_cnt / 3 / 32) + 1));
...
int CA3MPoly::CollisionSphere(CCollTriangle * out, const CVector3D & center, float radius, int maxcnt,const CVector3D& start, const CVector3D& end) {
	MY_VERTEX* v = nullptr;
	if (m_buffer) {
		glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
		v = (MY_VERTEX*)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_ONLY);
			...
			//	int ceil = ceil_x + lineX;
				...
				for (unsigned int i = 0, s = m_colIdex[ceil].size(); i < s; i++) {
					...
					if (CCollision::CollisionTriangleSphere(v[idx * 3].vPos, v[idx * 3 + 1].vPos, v[idx * 3 + 2].vPos, v[idx * 3].vNorm, center, radius, NULL, &dist)) {
						out->m_dist = dist;
						out->m_material_id = m_material;
						CVector3D e1 = v[idx * 3 + 1].vPos - v[idx * 3].vPos;
						CVector3D e2 = v[idx * 3 + 2].vPos - v[idx * 3].vPos;

						out->m_normal = CVector3D::Cross(e1, e2);
						out->m_vertex[0] = v[idx * 3 + 0].vPos;
						out->m_vertex[1] = v[idx * 3 + 1].vPos;
						out->m_vertex[2] = v[idx * 3 + 2].vPos;
...
int CA3MPoly::CollisionCupsel(CCollTriangle* out, const CVector3D& top, const CVector3D& bottom, float radius, int maxcnt,const CVector3D& start, const CVector3D& end) {
	MY_VERTEX* v = nullptr;
	if (m_buffer) {
		glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
		v = (MY_VERTEX*)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_ONLY);
		...
		for (int lineZ = start.iz; lineZ <= end.iz; lineZ++) {
			for (int lineX = start.ix; lineX <= end.ix; lineX++) {
				...
				for (unsigned int i = 0; i < m_colIdex[ceil].size(); i++) {
					int idx = m_colIdex[ceil][i];
					if (m_colmask[idx / 32] & (1 << (idx % 32))) continue;
					if (CCollision::CollisionTriangleCapsule(v[idx * 3].vPos, v[idx * 3 + 1].vPos, v[idx * 3 + 2].vPos, top, bottom, radius, NULL, &dist)) {
						out->m_dist = dist;
						out->m_material_id = m_material;
						CVector3D e1 = v[idx * 3 + 1].vPos - v[idx * 3].vPos;
						CVector3D e2 = v[idx * 3 + 2].vPos - v[idx * 3].vPos;
...
void CA3MMesh::Draw(std::vector<CMaterial*>& materialList, const CShader* shader, CMatrix* send_matrix, CMatrix* bone_matrix, int bone_size)
{
	glUniform1i(glGetUniformLocation(shader->GetProgram(), "useSkin"),*mp_enable_animation ? m_use_skin:0);
	...
	for (auto it = m_poly_list.begin(); it != m_poly_list.end(); it++) {
		CMaterial* m = materialList[(*it)->m_material];
		m->Map(shader);
		(*it)->Draw(shader);
		m->Unmap();
	}
...
int CA3MMesh::CollisionSphere(CCollTriangle* out, const CVector3D& center, float radius, int maxcnt, CMatrix& matrix)
{
	...
	for (auto it = m_poly_list.begin(); it != m_poly_list.end(); it++) {
		int c = (*it)->CollisionSphere(o, p, r, maxcnt,start,end);
		o += c;
		cnt += c;
		if (cnt > maxcnt) break;
...
int CA3MMesh::CollisionCupsel(CCollTriangle* out, const CVector3D& top, const CVector3D& bottom, float radius, int maxcnt, CMatrix& matrix)
{
	float scale = CVector3D(matrix.m00, matrix.m10, matrix.m20).Length();
	int cnt = 0;
	CCollTriangle* o = out;
	...
	for (auto it = m_poly_list.begin(); it != m_poly_list.end(); it++) {
		int c = (*it)->CollisionCupsel(o, t, b, r, maxcnt,start,end);
		o += c;
		cnt += c;
		if (cnt > maxcnt) break;
...
void CModelA3M::DrawMesh(CA3MNode* node,const CMatrix&view_matrix)
{
	...
	if (node->mp_mesh && node->m_visibility) {
		...
		//} else {
			glUniformMatrix4fv(glGetUniformLocation(m_shader->GetProgram(), "LocalMatrix"), 1, GL_FALSE, node->m_matrix.f);
			glUniformMatrix4fv(glGetUniformLocation(m_shader->GetProgram(), "ModelViewMatrix"), 1, GL_FALSE, (view_matrix* m_matrix).f);
			glUniformMatrix4fv(glGetUniformLocation(m_shader->GetProgram(), "WorldMatrix"), 1, false, m_matrix.f);
		//}
		m->Draw(m_material_list, m_shader, m_send_matrix,m_bone_matrix,m_bone_num);
	}
...
void CModelA3M::CalcBoneMatrix(CA3MNode* node)
{
	if (m_enable_animation) {
		...
		if (aa) {
			...
			node->m_local_matrix = key->m_matrix;
		}
		else {
...
void CModelA3M::operator=(const CModelA3M& m)
{
	...
	memcpy(m_send_matrix, m.m_send_matrix, sizeof(m_send_matrix));
	memcpy(m_bone_matrix, m.m_bone_matrix, sizeof(m_bone_matrix));
	memcpy(m_old_matrix, m.m_old_matrix, sizeof(m_old_matrix));
...
CModelA3M::CModelA3M(const CModelObj& m) : m_bone_num(0), m_bone_matrix(nullptr), m_animation_size(0),m_enable_animation(false)
{
	...
	for (int i = 0; i < m.m_materialCnt; i++) {
		...
		m_material_list.push_back(new CMaterial(m.m_mesh[i].m_material));
	...
	if (pNode->GetNodeAttribute()) {
		...
		case FbxNodeAttribute::eMesh:
		{
			...
			for (int i = 0; i < material_size; i++) {
				//m_vertex_cnt+m_material+m_vertex_cnt*MY_VERTEX;
				array_size += sizeof(unsigned long) + sizeof(unsigned long) +
					pMeshFBX->m_numFace[i]*3 * sizeof(MY_VERTEX);
			}
...
bool CModelA3M::DuplicateFBXAnimation(FbxNode* pNode,const CModelFBX::AnimationInfo *info,int anim_num)
{
	...
	if (!n->mp_mesh) {
		...
		aa->m_size = info->m_end - info->m_start+1;
		aa->m_keys = new CA3MAnimationKey[aa->m_size];

		int key_frame = info->m_start;
		...
		if (m_animation_list.size() <= anim_num) {
			...
			set->m_start = 0;
			set->m_fps = FbxTime::GetFrameRate(time_mode);
...
CModelA3M::CModelA3M(const CModelFBX& m): m_bone_num(0), m_bone_matrix(nullptr), m_animation_size(0)
{
	...
	if (m.GetAnimationSize() > 0) {
		...
		m_animation.push_back(&m_animation_list);
		m_animation.push_back(&m_animation_list);
		m_animation_size = m_animation_list.size();
...
CA3MNode* CModelA3M::DuplicateXMesh(const CModelX* model, CFrameBone* pNode, int& no, CA3MNode* parent, CA3MNode* prev)
{
	...
	if (pNode->GetType() == E_FRAME_TYPE::eMesh) {
		...
		int material_size = m->polyList.size();
		int use_skin = 0;
...
//	std::map<int, int> poly_cnt2;
	...
	if (pugi::xml_node face_node = pNode.child("Faces")) {
		...
		mesh->m_poly_cnt = mesh->m_poly_list.size();
...
void CModelA3M::LoadToyAnimationSet(pugi::xml_node& pNode,CA3MAnimationSet *set)
{
	...
	set->m_fps = atof(pNode.child("Fps").child_value());
	int start = atof(pNode.child("FirstFrame").child_value());
	int end = atof(pNode.child("LastFrame").child_value());
	int size = end - start+1;
	set->m_start = 0;
	set->m_end = set->m_end_base = size;
...
bool CModelA3M::Load(const char* filePath, int cut_x, int cut_y, int cut_z)
{
	...
	for (int i = 0; i < header.materialCnt; i++) {
		CMaterial* m = new CMaterial();
		fread(m, sizeof(SA3MaterialCHK), 1, fp);
...
int CModelA3M::WriteNode(FILE* fp, CA3MNode* node)
{
	...
	if (node->m_node_type == A3M::eMesh) {
		...
		unsigned int size = m->m_offset.size();
		fwrite(&size, sizeof(unsigned int), 1, fp);

		for(auto v:m->m_offset){
			SA3MBoneOffsetCHK of;
			of.no = v.m_bone_no;
...
void CModelA3M::Render(CMatrix& m)
{
	...
	m_shader->enable();
	UpdateMatrix(m);
	SendShaderParam(CMatrix::indentity, CCamera::GetCurrent()->GetViewMatrix(), CCamera::GetCurrent()->GetProjectionMatrix());
...
{
	if (m_animation.size() < layer+1) return true;
...
{
	if (m_animation.size() < layer + 1) return 0;
...
{
	if (m_animation.size() < layer + 1) return;
...
{
	return m_animation[0].GetAnimationFrame();
}
...
CMatrix CModelA3M::GetFrameMatrix(const char* name, bool local)const
{
	...
	if (!n) return CMatrix();
	return local ? n->GetMatrix() : m_matrix*n->GetMatrix();
}
...
int CModelA3M::CollisionSphere(CCollTriangle* out, const CVector3D& center, float radius, int maxcnt) const
{
	if (!m_col_cut) return 0;

	int cnt = 0;
	for (auto it = m_mesh_list.begin(); it != m_mesh_list.end(); it++) {
		int c = (*it)->mp_mesh->CollisionSphere(out, center, radius, maxcnt, m_matrix*(*it)->m_matrix);
		out += c;
		cnt += c;
		if (cnt > maxcnt) break;
...
int CModelA3M::CollisionCupsel(CCollTriangle* out, const CVector3D& top, const CVector3D& bottom, float radius, int maxcnt) const
{
	if (!m_col_cut) return 0;
	...
	for (auto it = m_mesh_list.begin(); it != m_mesh_list.end(); it++) {
		int c = (*it)->mp_mesh->CollisionCupsel(out, top, bottom, radius, maxcnt, m_matrix*(*it)->m_matrix);
		out += c;
		cnt += c;
		if (cnt > maxcnt) break;
...
bool CModelA3M::LoadTexture(const char* file_name, int material_num) {
	if (material_num >= m_material_list.size()) return false;
...
CA3MAnimation::CA3MAnimation(std::vector<CA3MAnimationSet*>* animation_list):m_jam(0),m_speed(1.0), m_loop(false), m_frame(0), m_blend(1.0), m_blend_rate(1.0)
{
...
bool CA3MAnimation::ChangeAnimation(int jam, bool loop, bool check, float time, float blendtime)
{
	if (check && jam == m_jam) return false;
	m_jam = jam;
	m_frame = time + (*mp_animation_list)[m_jam]->m_start;
	m_loop = loop;
	m_blend_rate = blendtime > 0 ? 1 / blendtime : 1.0f;
	m_blend = blendtime > 0 ? 0 : 1;
	return true;
...
void CA3MAnimation::UpdateAnimation()
{
	...
	if (m_frame > end) {
		if (m_loop) {
			m_frame = (*mp_animation_list)[m_jam]->m_start;
		}
		else {
			m_frame = end;
		}
