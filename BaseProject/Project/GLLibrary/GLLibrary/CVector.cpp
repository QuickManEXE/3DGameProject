...
CVector3D CQuaternion::GetEuler(int type) const {
	...
	if (fabs(sp) > 0.9999f) {
		v.x = M_PI / 2;
		v.y = atan2(-x * z - w * y, 0.5f - y * y - z * z);
		v.z = 0.0f;
	}
	else {
		v.x = asin(sp);
		v.y = atan2(x * z - w * y, 0.5f - x * x - y * y);
		v.z = atan2(x * y - w * z, 0.5f - x * x - z * z);
	}
	return v;
	*/
	CMatrix m;
...
CVector2D::CVector2D(const CVector3D& v)
{
	x = v.x; y = v.y;
}

CVector2D CVector2D::operator=(const CVector3D& v)
{
	x = v.x; y = v.y;
	return *this;
}
