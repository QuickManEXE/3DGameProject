...
public:
	...
	**/
	...
	/*
		...
		@retval	無し
	*/
	virtual void SendShaderParam(const CMatrix &m, const CMatrix &mv, const CMatrix &p);
	...
	**/
	...
	@retval	無し
	**/
	virtual void ChangeAnimation(int jam, bool loop = true, bool check = true, float time = 0, float blendtime = 0){ printf("非搭載"); }
	/*
	@brief	アニメーション数取得
	@retval	アニメーション数
	**/
	...
	**/
	virtual bool CollisionRay(CVector3D *c, CVector3D *n, const CVector3D &s, const CVector3D &e) const { printf("非搭載"); return false; }
	...
	@retval	接触ポリゴン数
	**/
	virtual int CollisionSphere(CCollTriangle *out, const CVector3D &center, float radius,int maxcnt=64) const { printf("非搭載"); return 0; }
	...
	@retval	接触ポリゴン数
	**/
	virtual int CollisionCupsel(CCollTriangle *out, const CVector3D &top, const CVector3D &bottom, float radius, int maxcnt = 64)const { printf("非搭載"); return 0; }
	...
	**/
	static CModel* CreateModel(const char *path, int cut_x = 1, int cut_y = 1, int cut_z = 1);
