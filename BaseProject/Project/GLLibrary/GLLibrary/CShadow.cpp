...
CShadow::CShadow(int screenWidth, int screenHeight,float length,int texWidth, int texHeight): m_state(eNone) {
	...
	glGenTextures(1, &m_depthtexBuffer);
	//深度バッファ格納用テクスチャーを作成

	glActiveTexture(GL_TEXTURE7);
	glBindTexture(GL_TEXTURE_2D, m_depthtexBuffer);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, m_depthtexWidth, m_depthtexHeight, 0,
	...
	glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D, m_depthtexBuffer, 0);
...
CShadow::~CShadow() {
	...
	glDeleteTextures(1,&m_depthtexBuffer);
...
void CShadow::Render(std::function<void()> render,bool shadow,bool autoCamera, bool edge, bool effect,bool clear) {
	...
	if (shadow) {
		...
		*c = *buf;
		...
		glViewport(0, 0, m_depthtexWidth, m_depthtexHeight);
...
void CShadow::DrawDepthTex(int x,int y,int w, int h)
{
	...
	mT.Transelate(x, y, 0.0f);

	pvm = mProj * mT * CMatrix::MScale(w, h, 1);;
